...
| STRE-04 | D-06 code → 한국어 메시지 테이블 | 5가지 코드 (mcp_timeout/mcp_offline/mcp_busy/stream_timeout/unknown)에 대해 각 한국어 메시지가 UI에 정확히 렌더 | (a) 타입: `error-messages.ts` KOREAN_MESSAGES 테이블이 5 entry 완비 (mcp_timeout 포함), client parseChatError 분기 커버 (b) 수동: 재현 가능한 실패 모드(주로 mcp_offline)는 로컬/프로덕션에서 유도 → UI 메시지 확인; 재현 불가한 코드(mcp_busy/stream_timeout/mcp_timeout)는 parseChatError code-path review로 검증 | 타입만 자동 |
...
| 인라인 에러 배너 + 재시도 버튼 렌더 (5개 코드 전체) | STRE-03 / STRE-04 / STRE-05 | React 상태 + AI SDK 스트림 생명주기가 얽혀 있어 유닛 테스트보다 Playwright E2E가 신뢰도 높음. 5개 코드(mcp_timeout/mcp_offline/mcp_busy/stream_timeout/unknown) 중 재현 가능한 것만 실측, 나머지는 parseChatError code-path review로 커버. | 로컬 Playwright로 LAW_API_KEY 비우거나 MCP URL 파괴 → mcp_offline 실패 턴 관찰 → "다시 시도" 버튼 클릭 → 재생성 확인. 5초 초과 지연은 mcp_timeout 경로를 트리거 (로컬 MCP URL을 blackhole로 바꿔 5초 타임아웃 유도). mcp_busy/stream_timeout/unknown은 parseChatError 테이블 lookup 정합성만 검증. |
