...
**Success criteria**
- [ ] 프로덕션에서 "근로기준법 제60조 연차휴가" 질문 시 답변 텍스트가 렌더링됨 (빈 카드 아님)
- [ ] 수정 전후 `console.log(messages[i].parts)` 진단 로그로 파트 구성 변화가 커밋 메시지에 기록됨
- [ ] `route.ts`에 `stopWhen: stepCountIs(8)`, `onFinish`/`onError`에서 MCP close, `consumeSseStream` + `onError` 콜백 적용
- [ ] 새 `lib/ui-message-parts.ts` 모듈에 `extractAssistantText` + 공식 type guard re-export 존재
- [ ] 새 `MessagePartRenderer` 컴포넌트가 `text` + `dynamic-tool` 최소 두 타입을 분기 처리 (나머지는 stub)
- [ ] `chat-container.tsx` / `test-sidebar/page.tsx` 양쪽의 inline `getMessageText` 제거
- [ ] 기존 localStorage 대화가 refactor 후에도 빈 카드 없이 렌더링됨 (read-time migration)
- [ ] Phase 1 종료 시 `finishReason: "stop"` (not `"tool-calls"`) 서버 로그로 확인
...
**Goal**
스트림 주변의 모든 실패 모드(타임아웃, MCP 혼잡, MCP 다운, Gemini 멀티턴 thought_signature, 일상 인사에서의 강제 툴 호출)가 사용자에게 명확하게 드러나고, 필요한 경우 재시도 가능하며, 내부 자원은 정리된다.

**Requirements covered**
- STRE-01 ~ STRE-09 (9개)

**Success criteria**
- [ ] `createMCPClient`가 5초 내 연결 안 되면 degraded mode로 fallback (도구 없이 진행)
- [ ] MCP `tools()` 스키마가 모듈 스코프에서 캐시됨 — 로컬 PoC로 Vercel warm-container 재사용 확인
- [ ] 에러 배너가 실패한 assistant bubble 내부에 inline 렌더링
- [ ] 3가지 실패 모드에 대한 구분된 한국어 메시지 노출
- [ ] 실패 턴에 "다시 시도" 버튼 (regenerate 호출)
- [ ] 트랜지언트 503 시 1회 자동 재시도 적용
- [ ] 시스템 프롬프트 완화 (일상 인사에 툴 호출 금지)
- [ ] Gemini 멀티턴 smoke test (3연속 툴 쿼리) 통과 또는 `thinkingBudget: 0` 적용
- [ ] `maxDuration` 값 결정 + PROJECT.md 업데이트 (60s 유지 근거 또는 300s 상향 근거)
...
**Goal**
사용자가 "어떤 법령을 찾고 있는지" 실시간으로 알 수 있고, 툴 호출 이력이 접히는 세부 블록에서 확인 가능한 상태.

**Requirements covered**
- TOOL-01 ~ TOOL-06 (6개)

**Success criteria**
- [ ] `ToolInvocationView`가 `DynamicToolUIPart`의 4가지 상태를 모두 분기 처리
- [ ] `tool-labels.ts` 맵이 4개 MCP 도구를 한국어로 매핑
- [ ] 동사 시제 상태 chip ("법령 검색 중: 근로기준법" → "검색 완료: 근로기준법")
- [ ] `<details>` 기반 접힘 블록, 기본 접힘 상태
- [ ] 여러 도구 호출이 세로 체크리스트로 스택됨
- [ ] 정적 "검색 중..." 문자열 제거됨
...
**Goal**
Chainlit 관련 코드/의존성/설정이 레포에서 완전히 사라지고, Slack 봇 경로는 여전히 정상 동작.

**Requirements covered**
- CLEAN-01 ~ CLEAN-06 (6개)

**Success criteria**
- [ ] `app.py`, `.chainlit/`, 관련 진입점 파일 모두 삭제
- [ ] `requirements.txt`에서 `chainlit` 제거 (Slack 봇 의존성은 유지)
- [ ] `Dockerfile` Chainlit 참조 제거 또는 Slack 봇 전용으로 재작성
- [ ] `frontend/src/app/test-sidebar/` 삭제
- [ ] `.env.example`에서 `CHAINLIT_AUTH_SECRET` 제거
- [ ] 수정 후 `python main.py` 수동 기동 테스트 — Slack 봇이 이벤트를 여전히 수신함
...
**Goal**
localStorage에 full `UIMessage[]`를 저장하고, 사이드바에서 과거 대화 선택 시 과거 assistant 턴이 (텍스트 + 도구 호출 트레이스 모두) 정상 렌더링된다.

**Requirements covered**
- PERS-01 ~ PERS-04 (4개)

**Success criteria**
- [ ] localStorage에 `{id, role, parts, metadata}` 구조로 저장됨 (flat string 제거)
- [ ] 저장 시 text part의 `providerExecuted` 제거, tool part의 `providerMetadata`는 보존
- [ ] 사이드바에서 과거 대화 선택 시 `useChat({id, messages: restored})` reseed 정상
- [ ] 과거 assistant 턴의 툴 호출 트레이스가 재렌더링됨
- [ ] 저장은 `status !== 'streaming'` 상태에서만 발동 (atomicity)
...
*Last updated: 2026-04-13 after initial creation*
